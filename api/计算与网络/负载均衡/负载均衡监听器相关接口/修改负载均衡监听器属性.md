...
|rstFlag|否|Int|传统型内网负载均衡TCP监听器属性参数。对于长链接的请求，解绑机器或者机器健康检查异常时，负载均衡的处理方式。 默认值0，当解绑机器或者机器健康检查异常时，长链接不会给客户端回复rst包，且对于该长链接的后续请求包依然发送给解绑的或者健康异常的这台机器，1 表示当解绑机器或者机器健康检查异常时都给客户端回复rst包， 2 表示解绑机器时发送rst包，但健康检查异常的时候不回复rst包。|
